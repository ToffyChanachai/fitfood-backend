...
const SetupMenuHhb = use("App/Models/SetupMenuHhb");

class SetupMenuHhbController {
     async index({ response }) {
        try {
          const menus = await SetupMenuHhb.query()
            .with("menu", (builder) => {
      ...
      async store({ request, response }) {
        ...
        const createdMenus = await SetupMenuHhb.createMany(menusWithDayOfWeek);
      ...
      async update({ params, request, response }) {
        ...
        const setupMenu = await SetupMenuHhb.findOrFail(id);
      ...
      async setStartDate({ request, response }) {
        ...
        try {
          ...
          let setting = await Setting.findBy("key", "start_date_hhb");
          if (setting) {
            setting.value = startDate;
          } else {
            setting = new Setting();
            setting.key = "start_date_hhb";
            setting.value = startDate;
          }
          await setting.save();
      ...
      async getStartDate() {
        const setting = await Setting.findBy("key", "start_date_hhb");
...
module.exports = SetupMenuHhbController
