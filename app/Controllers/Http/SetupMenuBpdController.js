...
const SetupMenuBpd = use("App/Models/SetupMenuBpd");

class SetupMenuBpdController {
  async index({ response }) {
    try {
      const menus = await SetupMenuBpd.query()
        .with("menu", (builder) => {
  ...
  async store({ request, response }) {
    ...
    const createdMenus = await SetupMenuBpd.createMany(menusWithDayOfWeek);
  ...
  async update({ params, request, response }) {
    ...
    const setupMenu = await SetupMenuBpd.findOrFail(id);
  ...
  async setStartDate({ request, response }) {
    ...
    try {
      ...
      let setting = await Setting.findBy("key", "start_date_bpd");
      if (setting) {
        setting.value = startDate;
      } else {
        setting = new Setting();
        setting.key = "start_date_bpd";
        setting.value = startDate;
      }
      await setting.save();
  ...
  async getStartDate() {
    const setting = await Setting.findBy("key", "start_date_bpd");
...
module.exports = SetupMenuBpdController;
