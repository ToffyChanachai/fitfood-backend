...
const SetupMenuDiabete = use("App/Models/SetupMenuDiabete");

class SetupMenuDiabeteController {
  async index({ response }) {
    try {
      const menus = await SetupMenuDiabete.query()
        .with("menu", (builder) => {
  ...
  async store({ request, response }) {
    ...
    const createdMenus = await SetupMenuDiabete.createMany(menusWithDayOfWeek);
  ...
  async update({ params, request, response }) {
    ...
    const setupMenu = await SetupMenuDiabete.findOrFail(id);
  ...
  async setStartDate({ request, response }) {
    ...
    try {
      ...
      let setting = await Setting.findBy("key", "start_date_diabete");
      if (setting) {
        setting.value = startDate;
      } else {
        setting = new Setting();
        setting.key = "start_date_diabete";
        setting.value = startDate;
      }
      await setting.save();
  ...
  async getStartDate() {
    const setting = await Setting.findBy("key", "start_date_diabete");
...
module.exports = SetupMenuDiabeteController;
