...
const SetupMenuLc = use("App/Models/SetupMenuLc");

class SetupMenuLcController {
  async index({ response }) {
    try {
      const menus = await SetupMenuLc.query()
        .with("menu", (builder) => {
  ...
  async store({ request, response }) {
    ...
    const createdMenus = await SetupMenuLc.createMany(menusWithDayOfWeek);
  ...
  async update({ params, request, response }) {
    ...
    const setupMenu = await SetupMenuLc.findOrFail(id);
  ...
  async setStartDate({ request, response }) {
    ...
    try {
      ...
      let setting = await Setting.findBy("key", "start_date_lc");
      if (setting) {
        setting.value = startDate;
      } else {
        setting = new Setting();
        setting.key = "start_date_lc";
        setting.value = startDate;
      }
      await setting.save();
  ...
  async getStartDate() {
    const setting = await Setting.findBy("key", "start_date_lc");
...
module.exports = SetupMenuLcController;
