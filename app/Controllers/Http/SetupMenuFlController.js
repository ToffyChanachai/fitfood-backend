...
const SetupMenuFl = use("App/Models/SetupMenuFl");

class SetupMenuFlController {
  async index({ response }) {
    try {
      const menus = await SetupMenuFl.query()
        .with("menu", (builder) => {
  ...
  async store({ request, response }) {
    ...
    const createdMenus = await SetupMenuFl.createMany(menusWithDayOfWeek);
  ...
  async update({ params, request, response }) {
    ...
    const setupMenu = await SetupMenuFl.findOrFail(id);
  ...
  async setStartDate({ request, response }) {
    ...
    try {
      ...
      let setting = await Setting.findBy("key", "start_date_fl");
      if (setting) {
        setting.value = startDate;
      } else {
        setting = new Setting();
        setting.key = "start_date_fl";
        setting.value = startDate;
      }
      await setting.save();
  ...
  async getStartDate() {
    const setting = await Setting.findBy("key", "start_date_fl");
...
module.exports = SetupMenuFlController;
