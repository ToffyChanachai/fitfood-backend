...
const SetupMenuFatDisease = use("App/Models/SetupMenuFatDisease");

class SetupMenuDiseaseController {
  async index({ response }) {
    try {
      const menus = await SetupMenuFatDisease.query()
        .with("menu", (builder) => {
      ...
      return response
        .status(500)
        .json({ message: "เกิดข้อผิดพลาดในการดึงข้อมูลเมนู" });
    }
  }
  ...
  async store({ request, response }) {
    ...
    const createdMenus = await SetupMenuFatDisease.createMany(
      menusWithDayOfWeek
    );
  ...
  async update({ params, request, response }) {
    ...
    const setupMenu = await SetupMenuFatDisease.findOrFail(id);
  ...
  async setStartDate({ request, response }) {
    ...
    try {
      ...
      let setting = await Setting.findBy("key", "start_date_fat_disease");
      if (setting) {
        setting.value = startDate;
      } else {
        setting = new Setting();
        setting.key = "start_date_fat_disease";
        setting.value = startDate;
      }
      await setting.save();
      ...
      return response
        .status(500)
        .json({ message: "เกิดข้อผิดพลาดในการอัปเดตวันเริ่มต้น" });
    }
  }

  async getStartDate() {
    const setting = await Setting.findBy("key", "start_date_fat_disease");
  ...
  async getMenusByDay({ params, response }) {
    try {
      ...
      if (menus.rows.length === 0) {
        return response
          .status(404)
          .json({ message: "No menus found for this day" });
      }
...
module.exports = SetupMenuDiseaseController;
