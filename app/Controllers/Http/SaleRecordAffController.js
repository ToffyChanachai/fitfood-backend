...
class SaleRecordAffController {
  ...
  async getDailySales({ request, response }) {
    try {
      ...
      const dailySales = await Database.from("sale_records_affs")
      ...
      const salesByPaymentType = await Database.from("sale_records_affs")
        .where("payment_status", "paid")
        .whereBetween("paid_date", [selectedStartDate, selectedEndDate])
  ...
  async getSaleRecordsByUserId({ auth, request, response }) {
    ...
    try {
      ...
      const query = SaleRecordAff.query().where("customer_id", customerId);
